...
`~/Projects/opensource/github.com`, then `codediff.nvim` with `dev = true` loads from
`~/Projects/opensource/github.com/fredrikaverpil/codediff.nvim`). The plugin's `.dir` field in the lazy API
