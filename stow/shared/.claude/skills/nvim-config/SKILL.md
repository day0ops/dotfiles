...
The native config lives at **`~/.dotfiles/nvim-custom/`** inside the dotfiles
repo. It is symlinked into place via GNU Stow:

```
~/.dotfiles/nvim-custom/          <- actual files (edit here)
~/.dotfiles/stow/shared/.config/nvim-custom -> ../../../nvim-custom  (stow entry)
~/.config/nvim-custom -> ~/.dotfiles/stow/shared/.config/nvim-custom  (stow result)
```
...
-- plugin/lang/go.lua
require("dev").use({
  dev = "~/Projects/opensource/github.com/fredrikaverpil/neotest-golang",
  fallback = function()
    vim.pack.add({
...
```sh
NVIM_APPNAME=nvim-custom nvim --startuptime /tmp/startup.log --headless +q
```
...
With `NVIM_APPNAME=nvim-custom`, paths use `nvim-custom` instead of `nvim`.
