...
targets_file="$(mktemp)"
titles_file="$(mktemp)"
trap 'rm -f "$targets_file" "$titles_file"' EXIT

# Every wikilink target in the vault, normalized to a bare title (strip
# "[[", display text after "|", and heading anchors after "#").
rg -o --no-filename --type md '\[\[[^]|#]+' -g '!.obsidian' -g '!.trash' . \
...
comm -23 "$targets_file" "$titles_file"
...
fd -tf -e md . -E .obsidian -E .trash -E _templates | while read -r file; do
  ...
  [[ ${#missing[@]} -gt 0 ]] && echo "$file: missing ${missing[*]}"
done
